...
def find_mode(da: DynamicArray) -> tuple[DynamicArray, int]:
    """
    Returns a tuple containing, in this order, a dynamic array comprising the mode value(s)
    of the given array, and an integer representing the highest frequency of occurrence for hte mode value(s).
    """
    map = HashMap()
    max_freq = 0
    ...
    result = DynamicArray()
    kv_pairs = map.get_keys_and_values()
    for i in range(kv_pairs.length()):
        key, val = kv_pairs[i]
        if val == max_freq:
            result.append(key)
